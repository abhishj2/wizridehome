...
export class BuyandsellcarComponent implements OnInit, OnDestroy {
  ...
  fetchTestimonials(): void {
    ...
    this.http.get<Testimonial[]>('http://cms.wizztest.com/wp-json/wp/v2/vehicle_testimonials?_embed&per_page=4&orderby=date&order=desc')
      .subscribe({
        next: (data) => {
  ...
  fetchFeaturedVehicles(): void {
    ...
    this.http.get<Vehicle[]>('http://cms.wizztest.com/wp-json/wp/v2/buy_sell_cars?status=publish&per_page=100&_embed')
      .subscribe({
        next: (data) => {
